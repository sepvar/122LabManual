...
view: 
	...
	@echo "/c/Program\ Files/Mozilla\ Firefox/firefox.exe ${LABSETUP}/TMC-lab-setup.html spring-lab-manual.html > /dev/null &"
...
122-h: ${MATHBOOK}/user/mathbook-lab-html.xsl src/122-Lab-Manual.ptx
	xsltproc --xinclude ${MATHBOOK}/user/mathbook-lab-html.xsl src/122-Lab-Manual.ptx
	@echo ""

spring-lab-manual.tex: ${MATHBOOK}/user/mathbook-lab-latex.xsl src/122-Lab-Manual.ptx
	xsltproc --xinclude ${MATHBOOK}/user/mathbook-lab-latex.xsl src/122-Lab-Manual.ptx
	sed -i.sedfix -f scripts/122-Lab-Manual.sed spring-lab-manual.tex

122-l: spring-lab-manual.tex

spring-lab-manual.pdf: spring-lab-manual.tex
	pdflatex spring-lab-manual.tex && pdflatex spring-lab-manual.tex || pdflatex spring-lab-manual.tex
...
checksize:
	...
	@grep "PDF version" src/122-Lab-Manual.ptx | sed 's#.*\">\(.*\)\.pdf (\([0-9][0-9]*\) kB.*#echo "echo -e \\\"claim: \2\\\\tactual:\\\" \\\\`expr `stat --printf=%s \1.pdf` / 1000\\\\`\\\\\t\1" >> scripts/list.two#g' >> scripts/list.one 
	scripts/list.one 
	scripts/list.two
...
fixsize:
	...
	@grep "PDF version" src/122-Lab-Manual.ptx | sed 's#.*\">\(.*\)\.pdf (\([0-9][0-9]*\) kB.*#echo "echo \\"s^\1.pdf (\2 kB)^\1.pdf (\\\\`expr `stat --printf=%s \1.pdf` / 1000\\\\` kB)^g\\"" >> scripts/step.two#g' >> scripts/step.one 
	scripts/step.one 
	scripts/step.two > scripts/step.sed
	sed -i.size -f scripts/step.sed src/122-Lab-Manual.ptx
	$(MAKE) checksize --no-print-directory

scripts/buildpdfs: spring-lab-manual.pdf
	@echo "Creating scripts/scripttolistbyname... (print at end)"
	@echo "#!/bin/sh" > scripts/scripttolistbyname
	@grep "chapter" spring-lab-manual.toc | \
		sed -n 'N;l;D' | \
		sed ':x ; $$!N ; s/\\\\\n// ; tx ; P ; D' | \
		grep -v "chapter\*" | \
		sed 's/\(.*\)\$$/\1/g' | \
		sed 's/.*{.*}{.*{.*}.\{3\}\(.*\)}{\([0-9][0-9]*\)}{\(.*\)}.*{.*}{.*{.*}.*}{\([0-9][0-9]*\)}{.*}/grep \"\1\\}\\\\\\\\\\\\\\\\label\" spring-lab-manual.tex \| sed \x22s#.\*label{\\\\(.\*\\\\)}#\3: `expr \2 + 12`..`expr \4 + 11`\\\\t\\\\1#g\x22 \| sed \x27s#c-##g\x27/g' \
	...
	@grep "chapter" spring-lab-manual.toc | \
		...
		grep -v "chapter\*" | \
		sed 's/\(.*\)\$$/\1/g' | \
		sed 's/.*{.*}{.*{.*}.\{3\}\(.*\)}{\([0-9][0-9]*\)}{\(.*\)}.*{.*}{.*{.*}.*}{\([0-9][0-9]*\)}{.*}/grep \"\1\\}\\\\\\\\\\\\\\\\label\" spring-lab-manual.tex \| sed \x22s#c-##g\x22 \| sed \x22s#.\*label{\\\\(.\*\\\\)}#pdfseparate -f `expr \2 + 12` -l `expr \4 + 11` spring-lab-manual.pdf \\\\1.\%d.pdf ; rm \\\\1_big.pdf \\\\1.pdf ; pdfunite \\\\1.*.pdf \\\\1_big.pdf ; rm \\\\1.*.pdf ; ps2pdf \\\\1_big.pdf \\\\1.pdf#g\x22/g' \
		>> scripts/buildscript
...
# @echo -e "I am going to run PDFsam, you should do this:\nextract pages listed\nmv PDFsam_spring-lab-manual.pdf measurement.pdf\nextract pages for StDev\nmv PDFsam_spring-lab-manual.pdf StDev.pdf\nmake checksize\nmake fixsize\nscripts/scripttolistbyname"
# @/c/Program\ Files\ \(x86\)/PDFsam\ Basic/bin/pdfsam.sh -e ./spring-lab-manual.pdf &

labpdf: scripts/buildpdfs

images: src/122-Lab-Manual.ptx ${LABSETUP}/src/Lab-setup.ptx
	${MATHBOOK}/script/mbx -v -c latex-image -f svg -d images ${DEE}/src/122-Lab-Manual.ptx
#	${MATHBOOK}/script/mbx -v -c latex-image -r [specific image reference] -f svg -d images ${DEE}/src/122-Lab-Manual.ptx
...
list: src/122-Lab-Manual.ptx ${LABSETUP}/src/Lab-setup.ptx
	...
	@echo "*************************"
	cat src/122-Lab-Manual.ptx | \
		sed 's/^ *<image/<image/g' | \
		grep '<image' | grep -v "images" | \
		sed 's/ width=.*>/>/g' | \
		sed 's+^.*ptx:id=\"\(.*\)\">+ls images/\1.svg || C:/Users/tensen/Desktop/Book/mathbook/script/mbx \-v \-c latex-image \-r \1 \-f svg \-d images ${PHY122L}/src/122-Lab-Manual.ptx+g'

counterr: ${MATHBOOK}/../jing-trang/build/jing.jar ${MATHBOOK}/schema/pretext.rng src/122-Lab-Manual.ptx  ${LABSETUP}/src/Lab-setup.ptx
	@echo `java -jar ${MATHBOOK}/../jing-trang/build/jing.jar ${MATHBOOK}/schema/pretext.rng ${LABSETUP}/src/Lab-setup.ptx | wc -l`" errors"
	@echo `java -jar ${MATHBOOK}/../jing-trang/build/jing.jar ${MATHBOOK}/schema/pretext.rng src/122-Lab-Manual.ptx | wc -l`" errors"

toperr: ${MATHBOOK}/../jing-trang/build/jing.jar ${MATHBOOK}/schema/pretext.rng src/122-Lab-Manual.ptx  ${LABSETUP}/src/Lab-setup.ptx
	java -jar ${MATHBOOK}/../jing-trang/build/jing.jar ${MATHBOOK}/schema/pretext.rng ${LABSETUP}/src/Lab-setup.ptx | head -5
	@echo "*************************"
	java -jar ${MATHBOOK}/../jing-trang/build/jing.jar ${MATHBOOK}/schema/pretext.rng src/122-Lab-Manual.ptx | head -5

typeerr: ${MATHBOOK}/../jing-trang/build/jing.jar ${MATHBOOK}/schema/pretext.rng src/122-Lab-Manual.ptx  ${LABSETUP}/src/Lab-setup.ptx
...
# 	java -jar ${MATHBOOK}/../jing-trang/build/jing.jar ${MATHBOOK}/schema/pretext.rng src/122-Lab-Manual.ptx | grep ": element \"todo" | sed 's/.*:\([0-9][0-9]*\):\([0-9][0-9]*\):.*/todo line \1:\2/g'
#                                                                                                          ^^^^                                                 ^^^^

allerr: ${MATHBOOK}/../jing-trang/build/jing.jar ${MATHBOOK}/schema/pretext.rng src/122-Lab-Manual.ptx  ${LABSETUP}/src/Lab-setup.ptx
	java -jar ${MATHBOOK}/../jing-trang/build/jing.jar ${MATHBOOK}/schema/pretext.rng ${LABSETUP}/src/Lab-setup.ptx | \
		sort -k4  
	@echo "*************************"
	java -jar ${MATHBOOK}/../jing-trang/build/jing.jar ${MATHBOOK}/schema/pretext.rng src/122-Lab-Manual.ptx | \
